...
Use `DroidSwarm brain ...` and the project-scoped `.agent/` folder for portable agent-brain work. If `DROIDSWARM_ENABLE_AGENTIC_BRAIN=true` is enabled, memory promotion, nightly dream cycles, progressive skill disclosure, and skill rewrite staging should route through the shared agent-brain surfaces rather than bespoke files or hidden local state.
