...
2. **Orchestrator**: ensure every scheduler decision calls the `TaskSchedulerEvents` hooks so `OrchestratorEngine` can convert them back into WebSocket envelopes (`plan_proposed`, `checkpoint_created`, `verification_requested`, `verification_completed`).
