...
const interruptedAttemptStatuses = ["running", "blocked"];
class RunLifecycleService {
  ...
  markInterruptedAttemptsFailed(runId, reason) {
    for (const status of interruptedAttemptStatuses) {
      const rows = this.persistence.database.prepare("SELECT attempt_id FROM task_attempts WHERE run_id = ? AND status = ?").all(runId, status);
