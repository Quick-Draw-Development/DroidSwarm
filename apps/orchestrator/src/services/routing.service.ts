...
export class RoutingService {
  ...
  decide(task: PersistedTask, role: string, policy?: TaskPolicy): RoutingDecision {
    ...
    const decision = this.routing.decide({
      ...
      iterationCountExpected: typeof task.metadata?.expected_iterations === 'number' ? task.metadata.expected_iterations : undefined,
      selfCorrectionNeeded: task.metadata?.self_correction_needed === true,
      longHorizon: task.metadata?.long_horizon === true,
      polishingPhase: task.metadata?.polishing_phase === true,
      failureRecoveryMode: task.metadata?.failure_recovery_mode === true,
